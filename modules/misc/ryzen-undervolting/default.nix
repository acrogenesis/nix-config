...
in {
  options.services.ryzen-undervolting = {
    ...
    coreCount = lib.mkOption {
      description =
        "The cores to which the offset should be applied (0..corecount)";
      type = lib.types.int;
      default = 8;
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      ...
      }
      {
        ...
        message =
          "service.ryzen-undervolting.coreCount has to be a number between 1 and 16";
      }
    ];
    environment.systemPackages = [ ryzen-undervolting ];
    ...
    systemd.services.ryzen-undervolting = {
      ...
      serviceConfig = {
        Type = "oneshot";
        ExecStart =
          "${lib.getExe pkgs.python3} ${lib.getExe ryzen-undervolting} -c ${
            builtins.toString cfg.coreCount
          } -o ${builtins.toString cfg.offset}";
      };
    };
  };
}
