...
let cfg = config.services.fail2ban-cloudflare;
in {
  options.services.fail2ban-cloudflare = {
    enable = lib.mkEnableOption { description = "Enable fail2ban-cloudflare"; };
    apiKeyFile = lib.mkOption {
      description =
        "File containing your API key, scoped to Firewall Rules: Edit";
      type = lib.types.str;
    ...
    zoneId = lib.mkOption { type = lib.types.str; };
    jails = lib.mkOption {
    ...
    environment.etc = lib.attrsets.mergeAttrsList [
        ...
        }))) cfg.jails)
      {
        "fail2ban/action.d/cloudflare-token-agenix.conf".text = let
          ...
          cfapi =
            "https://api.cloudflare.com/client/v4/zones/${cfg.zoneId}/firewall/access_rules/rules";
        in ''
          [Definition]
          actionstart =
          actionstop =
